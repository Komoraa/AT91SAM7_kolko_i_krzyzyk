...
void rysuj_tablice(char kolko_i_krzyzyk[3][3]){ //funkcja rysuj¹ca ca³¹ tablicê
LCDClearScreen();
for(int i=0;i<3;i++){
      for(int j=0;j<3;j++){
...
while(1){ //pêtla gry
  LCDClearScreen();
  char ster='o'; //ustawienie aktualnego gracza jako kó³ko 
  int i=0;
  ...
  while(1){
    ...
    while(1){
     if( ((PIOA_PDSR & PIOA_SODR_P14 ) == 0) && x<2){  //if przesuwajacy wskaznik w prawo
       rysuj_znak(kolko_i_krzyzyk,x,y); //usuniecie wskaŸnika z poprzedniej pozycji
       x+=1;
       delay(1000);
      }
